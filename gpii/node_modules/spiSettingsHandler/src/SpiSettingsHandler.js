...
gpii.windows.spi.setImpl = function (payload) {
    ...
    if (payload.settings.HighContrastOn) {
        ...
        if (doNothing) {
            ...
            var optsCachedFiles = { "recursive": true, "silent": true, "local": true };

            var tWallpaperPath = path.join(process.env.APPDATA, "Microsoft\\Windows\\Themes\\TranscodedWallpaper");
            var optsWallpaper = { "silent": true, "local": true };

            gpii.windows.rm(cachedFilesPath, optsCachedFiles);
            gpii.windows.rm(tWallpaperPath, optsWallpaper);
        }
    }
